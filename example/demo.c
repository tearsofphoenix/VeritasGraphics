...
void drawLabel(NVGcontext* vg, const char* text, float x, float y, float w, float h)
{
    ...
    VGLabelRef label = VGLabelCreate();
    VGLabelSetTitle(label, text);
    VGLayerSetFrame(label, VGRectMake(x, y, w, h));
...
static VGInputRef drawEditBoxBase(NVGcontext* vg, float x, float y, float w, float h)
{
    VGInputRef input = VGInputCreate();
    VGLayerSetFrame(input, VGRectMake(x, y, w, h));
...
{
	VGInputRef input = drawEditBoxBase(vg, x,y, w,h);

    VGInputSetPlaceholder(input, text);
...
void drawSpinner(NVGcontext* vg, float cx, float cy, float r, float t)
{
    ...
    VGLayerSetFrame(spinner, VGRectMake(cx - r, cy - r, 2 * r, 2 * r));
